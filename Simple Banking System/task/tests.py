...
def get_credentials(output: str, count=0):
    ...
    if not number:
        return CheckResult.wrong('You are printing the card number incorrectly. '
    ...
    if not PIN:
        return CheckResult.wrong('You are printing the card PIN incorrectly. '
                                   'The PIN should look like in the example: DDDD, where D is a digit.\n'
                                   'Make sure the PIN is 4-digit length and you don\'t print any extra spaces at the'
                                   ' end of the line!')
    if count == 2:
        if len(number) == 1 or len(PIN) == 1:
            return CheckResult.wrong(f'Expected 2 generated card credentials, but found {len(number)}')
